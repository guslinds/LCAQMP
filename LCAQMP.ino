...
void setup() {
  ...
  //we have to assing uart functionality to pin 11 and 10 since we have defined them as it above
...
void printValues() {
  ...
  String parsedValues = String(millis()) + ", " + getSDS011StringLastReading() + "," + getBME680StringLastReading() + "," + getCCS811StringLastReading() + "," + getGPSStringLastReading() + "," + getCosIRStringLastReading() + ", ERRORS : " + errorString;

  //print the csv row
  printSD(parsedValues);
  printSerial(getMillisStringHeader() + ", " + getSDS011StringHeader() + ", " + getBME680StringHeader() + "," + getCCS811StringHeader() + "," + getGPSStringHeader() + "," + CozIrStringHeader());
...
void printSD(String input) {
  ...
  if (SD.begin(cardSelect)) {
    if (!sdCardConnected) {
      ...
      printSDAndSerial("the connection to the sd card was lost but is now reesablished!");
    }
